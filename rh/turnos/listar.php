...
<table id="html5-extension" class="table table-hover non-hover" style="position: relative; z-index: 100;">
	...
	<tbody>
		...
		if($txtpesquisar == ''){
		$res = $pdo->query("SELECT * from turnos order by id");
	}else{
		$txtpesquisar = '%'.@$_POST['txtpesquisar'].'%';
		$res = $pdo->query("SELECT * from turnos  where cargo LIKE '$txtpesquisar' order by id asc");
	}
	
	$dados = $res->fetchAll(PDO::FETCH_ASSOC);

	//TOTALIZAR OS REGISTROS PARA PAGINAÇÃO
		$res_todos = $pdo->query("SELECT * from turnos");
